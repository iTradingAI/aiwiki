...
- 刷新 Core 1.0 ROADMAP 交付态，记录已随 1.0.0 交付的工作，并将面向使用者的版本声明同步至 1.0.1。
- 本次为仅文档发布，不包含行为变更。

## [1.0.0] - 2026-08-26

- 冻结 Core 1.0 契约矩阵及其契约测试覆盖。
- 补充现有工作区的人工复核优先迁移指南、完整安全指南与插件签名提案；签名仍仅为提案，当前不提供签名验证。
- 将 Core 1.0 兼容矩阵纳入 `npm run test:compat`。
