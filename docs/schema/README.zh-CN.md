...
- `aiwiki health --json` 与 `aiwiki repair --plan --json` 分别输出附加的 `aiwiki.health.v1` 和 `aiwiki.repair_plan.v1` 合同。两者均为只读：不会修改 Markdown、构建 state 或创建 dashboard。持久 Health Report 与 Core 0.5 发布门禁由 `CORE-0506` 单独负责。
