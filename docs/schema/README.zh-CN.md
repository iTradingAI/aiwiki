...
| Extension 作者合同 | `aiwiki.extension.v1` | 公开包合同 | 声明 API 已启用；加载仍由 CORE-0405 负责。 |

`aiwiki.context.v2` 仍为预留值。`aiwiki.extension.v1` 作为仅声明的 [Extension API v0.1](EXTENSION_SCHEMA.zh-CN.md) 已启用；Core 不会输出 Context v2，也不会加载扩展包。
