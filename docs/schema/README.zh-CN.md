...
`aiwiki.context.v2` 仍为预留值。`aiwiki.extension.v1` 作为 [Extension API v0.1](EXTENSION_SCHEMA.zh-CN.md) 已启用。CORE-0405 只增加显式的 [Extension Host v0.1](EXTENSION_HOST.zh-CN.md)：通过 `plugin add` 与 `plugin enable` 启用 local 或 bundled extension；没有自动发现。
