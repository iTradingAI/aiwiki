...
AIWiki Core 通过统一目录记录当前数据合同。该目录是内部兼容性边界，不是新的 CLI 能力，也不是 Extension API。
...
`aiwiki.context.v2` 与 `aiwiki.extension.v1` 仅为预留值。Core 不会输出 Context v2，不提供 Extension API，也不会加载扩展。
...
Schema 目录不会新增自然语言意图、命令或自动 Skill 匹配。CORE-0403 保持既有的命令优先匹配不变。CORE-0407 负责后续 Skill 匹配合同，必须在改变行为前定义示例、优先级、fallback 和验收测试。
