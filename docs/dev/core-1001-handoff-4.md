# CORE-1001 Slice 4 Linux gate handoff

Run the packed-package compatibility gate from a clean temporary consumer directory. Keep `AIWIKI_HOME`, `HOME`, `USERPROFILE`, and `CODEX_HOME` under that temporary directory so the gate neither reads nor writes a developer's real configuration.
...
```sh
node --test tests/contracts/cli-compatibility.test.ts
```

The contract test packs and installs the current package before executing these boundaries; do not replace it with a source-tree CLI invocation.
