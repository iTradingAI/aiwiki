...
> **历史归档：** 本文保留 2026-06-07 当时的运营判断，不是当前产品路线图。当前边界请以 `ROADMAP.zh-CN.md` 与根目录 CHANGELOG 为准；以下事实正文保持原样。原始 Git blob：`ded63d9c301bb46de9516296076abb19d1ed837e`。
...
- 开发任务进入标准。
- 不值得做的功能清单维护。
- 低反馈阶段的继续推进方式：跑试用任务、发布案例、问窄问题，并把沉默记录为试用步骤信号。
- 明确不向 CLI 增加遥测；未来如需遥测，必须单独设计 opt-in 任务。
